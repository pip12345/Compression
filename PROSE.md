...
An example for an improvement would be the usage of `std::vector<bool>` instead of binary representing strings or some form
of dynamic implementation for `std::bitset`. Both of these would represent bits as actual individual bits instead of having a full byte
overhead.
